...
int pagerankMonolithicOmpLoop(vector<T>& a, vector<T>& r, vector<T>& c, const vector<T>& f, const vector<size_t>& vfrom, const vector<K>& efrom, K i, K n, K N, T p, T E, int L, int EF) {
  T  c0 = (1-p)/N;
  int l = 0;
  while (l<L) {
    ...
    multiplyValuesOmp(a, f, c, i, n);                    // update partial contributions (c)
